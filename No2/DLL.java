public class DLL {
    ...
    public void print() {
        Node current = head;
        System.out.printf("| %-10s| %-12s| %-12s| %-12s| %-7s| %-12s %s|%n", "NOMOR TNKB", "NAMA PEMILIK", "JENIS",
                "CC KENDARAAN", "TAHUN", "BULAN HARUS BAYAR", "");

        while (current != null) {
            System.out.printf("| %-10s| %-12s| %-12s| %-12s| %-7s| %-12s %s|%n", current.data.kendaraan.noTNKB,
                    current.data.kendaraan.nama, current.data.kendaraan.jenis,
                    current.data.kendaraan.cc, current.data.kendaraan.tahun, current.data.kendaraan.bulanHarusBayar,
                    "");
            ...
            // System.out.println("Bulan Harus Bayar: " +
            // current.data.kendaraan.bulanHarusBayar);
            // System.out.println();
            current = current.next;
        }
    }
